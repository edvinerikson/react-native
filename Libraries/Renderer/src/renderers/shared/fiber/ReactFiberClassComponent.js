...
module.exports = function (scheduleUpdate: (fiber: Fiber) => void) {
  ...
  const updater = {
    ...
    enqueueSetState(instance, partialState) {
      ...
      const updateQueue = fiber.updateQueue ? addToQueue(fiber.updateQueue, partialState) : createUpdateQueue(partialState);
    ...
    enqueueCallback(instance, callback) {
      const fiber = ReactInstanceMap.get(instance);
      let updateQueue = fiber.updateQueue ? fiber.updateQueue : createUpdateQueue(null);
  ...
  function checkShouldComponentUpdate(workInProgress, oldProps, newProps, newState, newContext) {
    ...
    if (typeof instance.shouldComponentUpdate === 'function') {
      ...
      if (__DEV__) {
        warning(shouldUpdate !== undefined, '%s.shouldComponentUpdate(): Returned undefined instead of a ' + 'boolean value. Make sure to return true or false.', getComponentName(workInProgress));
      }
  ...
  function checkClassInstance(workInProgress: Fiber) {
    ...
    if (__DEV__) {
      ...
      const noGetInitialStateOnES6 = !instance.getInitialState || instance.getInitialState.isReactClassApproved;
  ...
  // could be reused.
  function resumeMountClassInstance(workInProgress: Fiber): bool {
  ...
  function updateClassInstance(current: Fiber, workInProgress: Fiber): bool {
    const instance = workInProgress.stateNode;
