...
module.exports = function <T, P, I, TI, C>(config: HostConfig<T, P, I, TI, C>) {
  ...
  const { commitInsertion, commitDeletion, commitWork, commitLifeCycles } = ReactFiberCommitWork(config, trapError);
  ...
  let shouldBatchUpdates: bool = false;

  // Need this to prevent recursion while in a Task loop.
  let isPerformingTaskWork: bool = false;
  ...
  let isAnimationCallbackScheduled: bool = false;
  let isDeferredCallbackScheduled: bool = false;
  ...
  function trapError(failedFiber: Fiber | null, error: any, isUnmounting: bool): void {
      ...
      if (parent) {
        if (parent.tag === ClassComponent && boundary === null) {
          ...
          if (isBoundary) {
            ...
            const isBoundaryAlreadyHandlingAnotherError = activeErrorBoundaries !== null && activeErrorBoundaries.has(parent);
