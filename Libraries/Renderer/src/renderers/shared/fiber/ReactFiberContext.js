...
const didPerformWorkStack: Array<bool> = [];
...
exports.hasContextChanged = function (): bool {
  return index > -1 && didPerformWorkStack[index];
};

exports.isContextProvider = function (fiber: Fiber): bool {
...
exports.pushContextProvider = function (fiber: Fiber, didPerformWork: bool): void {
  ...
  if (canReuseMergedChildContext) {
    ...
    for (let contextKey in childContext) {
      invariant(contextKey in childContextTypes, '%s.getChildContext(): key "%s" is not defined in childContextTypes.', getComponentName(fiber), contextKey);
    }
    if (__DEV__) {
      const name = getComponentName(fiber);
      const debugID = 0; // TODO: pass a real ID
